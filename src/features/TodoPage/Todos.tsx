...
const Todos: React.FC<unknown> = () => {
  ...
  const status = current.matches('init')
    ? 'loading...'
    : current.matches('ready.saving')
    ? 'saving...'
    : '';
  ...
  return (
    <TodosComponent
      disableAddButton={diabledAddButton}
      status={status}
      onClickAdd={() => send({ type: 'TODOS.ADD', value: 'foo' })}
    >
      {todoRefs?.map((todo, index) => (
        <Todo key={todo.id} todoRef={todo} />
      ))}
    </TodosComponent>
  );
};
